...
const_fit_plot <- plot_data_and_model(ex1$dat$y, constant_fit$zest, mu = constant_fit$mu)
clean_const_plot <- const_fit_plot +
  labs(title = NULL, x = NULL, y = NULL) +      # drop title & axis labels
  theme_minimal() +                             # white background
  theme(
...
ggsave(
  filename = "const_fit_clean.png",
  plot     = clean_const_plot,
  width    = 1026, height = 673,                # exact LaTeX size
  units    = "px",
  dpi      = 100,
  bg       = "white"
)
...
kernel_em_fit <- kernel_em(ex1$dat$y, K = 2, hmu = 13, hSigma = 7, hpi = 10, num_iter = 20)
...
clean_and_save(kernel_em_fit_plot, "kernel_fit.png")
...
clean_and_save(hung_plot, "hungarian_fit_dis.png")
...
## ------------------------------------------------------------------
intersect_cluster <- 1            # ← change this number as needed
